################################################################################
# SCALING WATERSHED FUNCTION: RAW DATA REVISION
################################################################################
...
bgl23_dat <- read_csv(paste(raw_data,"230423_enhanced_nhdp_2_yrb_wrb.csv", sep = '/'),
...
wcz21_dat <-  read_csv(paste(raw_data,"230428_pnw_basin_characteristics.csv", sep = '/'),
                      show_col_types = FALSE)
...
sch19_dat <- read_csv(paste(raw_data,"230423_main_nhdp_2_yrb_wrb.csv", sep = '/'),
                       show_col_types = FALSE)
...
# Comparing Areas, slopes, and stream lengths between the datasets:
# Hydrological data (Schwarz_19_Ancillary_Attributes)
# Physical characteristics of the river basins (Wieczeroek_21_Select_Attributes)
# enhanced NHDPlus V.2. as the reference dataset for COMIDs (Blodgett_23_Network_Attributes)

phys_comp <-bgl23_dat %>% #(Blodgett 2023)
  select(comid,
         areasqkm,
         slope,
         lengthkm,
...
a_comp_plot <- phys_comp %>% 
  select(comid,
         streamorde,
         CatAreaKm2,
         CAT_BASIN_AREA,
         areasqkm) %>% 
  rename(sch19_ancillaryh_area = CatAreaKm2,
         wcz21_attributes_area = CAT_BASIN_AREA,
  ...
  gather(.,
         ...
         factor_key = TRUE,
         c(4:5)) %>% 
  ggplot(aes(x = sch19_ancillaryh_area,
...
# The two most recent datasets, wzc21 and blg23 are in good agreement with the 
# watershed area values in the sch19 dataset. However, wcz21 truncate catchment 
# areas below 0.01km2, while  sch19 (and by corolary blg23) assign values below
# such threshold, and quite close to zero. 

# Let's take a look at the data for which catchment areas are below 0.01km2

summary(filter(phys_comp,areasqkm<0.01))

# There is a total of 543 datapoints with catchment areas less than 0.01km2, including
# missing values. 

# A quick verification: 

p <- ggplot(data = phys_comp,
            aes(x = areasqkm,
                y = CAT_BASIN_AREA))+
  geom_point()+
  labs(x = "blg23_network_at_area (km2)",
       y = "wcz21_attributes_area (km2)")+
  scale_y_log10()+
  scale_x_log10()+
  geom_abline()
p
...
s_comp_plot <- phys_comp %>% 
  select(comid,
         streamorde,
         SLOPE,
         CAT_STREAM_SLOPE,
         slope) %>% 
  rename(sch19_ancillaryh_slope = SLOPE,
         wcz21_attributes_slope = CAT_STREAM_SLOPE,
  ...
  gather(.,
         ...
         factor_key = TRUE,
         c(4:5)) %>% 
  ggplot(aes(x = sch19_ancillaryh_slope,
             y = flowline_slope,
             color = as.factor(streamorde)))+
  geom_point()+
  scale_x_log10()+
...
# The slopes from the blg23_network_at dataset are in relatively better agreement with the 
...
p <- ggplot(filter(phys_comp, SLOPE > 0.0001), 
            aes(x = SLOPE,
                fill = as.factor(streamorde),
                color = as.factor(streamorde)))+
  scale_x_log10()+
  xlab("sch19_ancillaryh_slope")+
...
p1 <- ggplot(filter(phys_comp, CAT_STREAM_SLOPE > 0.0001), 
            aes(x = CAT_STREAM_SLOPE,
                fill = as.factor(streamorde),
                color = as.factor(streamorde)))+
  scale_x_log10()+
  xlab("wcz21_attributes_slope")+
  geom_density(alpha = 0.5)+
  facet_wrap(~as.factor(huc_4), ncol = 2)
p1


p2 <- ggplot(filter(phys_comp, slope > 0.0001), 
             aes(x = slope,
                 fill = as.factor(streamorde),
                 color = as.factor(streamorde)))+
  scale_x_log10()+
  xlab("blg23_network_at_slope")+
  geom_density(alpha = 0.5)+
  facet_wrap(~as.factor(huc_4), ncol = 2)
p2
...
cat_areas <- ggplot(data = phys_comp,
...
test_dat <- filter(bgl23_dat,slope>0.000001)
