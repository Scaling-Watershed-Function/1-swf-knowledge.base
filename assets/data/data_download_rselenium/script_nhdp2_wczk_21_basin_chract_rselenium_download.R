...
# Merging Bankfull Data with Basin Characteristics CONUS

conus_dat <- merge(bsn_chr_data,
                   bnkfl_dat,
                   by = "COMID",
                   all.x = TRUE)

write.csv(conus_dat,paste(raw_data,"230521_conus_basin_characteristics.csv",sep = '/'),
                       row.names = FALSE)

Bbsn_chr_ext <- bsn_dat %>% 
  merge(.,
        bnkfl_dat,
        by.x = "comid",
        by.y = "COMID",
