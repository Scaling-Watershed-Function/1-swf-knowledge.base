...
nlcd_2001_dat <- read_csv(paste(source_data,"nlcd_2001_v2019","raw_data","nlcd_2001_v2019_NLCD01_TOT_CONUS.csv", sep = '/'),
                          show_col_types = FALSE)
...
rcm_22_connectivity <- rcm_22_model_dat %>% 
  ...
  mutate(inc_comid = 1,
         accm_inc_comid = calculate_arbolate_sum(data.frame(ID = comid,
                                                            toID = tocomid,
                                                            length = inc_comid)),
         connectivity_index = (max(accm_inc_comid)/sum(inc_comid))*100) %>% 
  ungroup()
...
# Internet database and re-calculate connectivity
...
nsi_rcm_ntwk_dat_test <- nsi_rcm_ntwk_dat %>% 
  filter(TotDASqKM > 0) %>%
...
p <- ggplot(data = nsi_rcm_phys_dat_0,
            aes(x = wshd_area_km2,
                y = 10^logw_m))+
  scale_x_log10()+
  scale_y_log10()+
...
hyporheic_predictors <- as.data.frame (nsi_rcm_phys_dat %>% 
  select(-geometry)) %>% 
